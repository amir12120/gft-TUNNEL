#!/usr/bin/env bash
# ============================================================
# GRE+FRP-TUNNEL — bootstrap installer
#
# From a checkout:      sudo ./install.sh
# From GitHub:          curl -fsSL https://raw.githubusercontent.com/amir12120/GRE-FRP-TUNNEL/main/install.sh | sudo bash
#
...
REPO_NAME="GRE-FRP-TUNNEL"
REPO_URL="${GRE_FRP_REPO_URL:-https://github.com/${REPO_OWNER}/${REPO_NAME}.git}"
INSTALL_DIR="${GRE_FRP_DIR:-/opt/gre-frp-tunnel}"
BIN_DIR="${GRE_FRP_BIN_DIR:-/usr/local/bin}"
...
if [ -n "$SELF_DIR" ] && [ -f "$SELF_DIR/gre-frp-tunnel" ]; then
  SRC_DIR="$SELF_DIR"
...
if [ -z "$SRC_DIR" ]; then
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    die "$INSTALL_DIR exists but is not a git checkout — remove it or set GRE_FRP_DIR"
  else
    say "cloning $REPO_URL → $INSTALL_DIR"
    have git || die "git is required to clone the repository"
    git clone --depth 1 "$REPO_URL" "$INSTALL_DIR" >/dev/null 2>&1 \
      || die "git clone failed — check the URL or set GRE_FRP_REPO_URL"
  fi
  SRC_DIR="$INSTALL_DIR"
fi

[ -f "$SRC_DIR/gre-frp-tunnel" ] || die "gre-frp-tunnel not found in $SRC_DIR"

# -----------------------------------------------------------
# Put it on PATH
# -----------------------------------------------------------
chmod +x "$SRC_DIR/gre-frp-tunnel" "$SRC_DIR/install.sh" 2>/dev/null || true
chmod +x "$SRC_DIR"/lib/*.sh 2>/dev/null || true
mkdir -p "$BIN_DIR"
ln -sfn "$SRC_DIR/gre-frp-tunnel" "$BIN_DIR/gre-frp-tunnel"
ln -sfn "$SRC_DIR/gre-frp-tunnel" "$BIN_DIR/gft"
ok "installed: $BIN_DIR/gre-frp-tunnel (alias: gft)"

# -----------------------------------------------------------
# Hand over to the CLI
# -----------------------------------------------------------
if [ "$#" -gt 0 ]; then
  exec "$SRC_DIR/gre-frp-tunnel" "$@"
fi
exec "$SRC_DIR/gre-frp-tunnel" install
